        ...
        {months.map((month) => {
          ...
          return (
            <Paper key={monthKey} sx={{ p: 1 }}>
              ...
              >
                {Array.from({ length: startOffset + daysInMonth }).map(
                  (_, idx) => {
                    const dayIndex = idx - startOffset + 1;
                    const isValid = dayIndex >= 1;
                    const dayDate = isValid
                    ...
                    const isToday =
                      isValid &&
                      dayjs(dayDate).format("YYYY-MM-DD") === todayString;

                    const dayEvents = isValid
                      ? events.filter(
                          (e): e is Event =>
                            !!e.startDate &&
                            dayjs(e.startDate).isSame(dayDate, "day")
                        )
                      : [];
      ...
      {dayPopover.anchor && dayPopover.date && (
        <DayEventsPopover
          ...
          events={events.filter(
            (e): e is Event =>
              !!e.startDate && dayjs(e.startDate).isSame(dayPopover.date, "day")
          )}
